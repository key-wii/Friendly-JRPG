/// @desc scr_earthbound_bg_set_wave_y(instance_id, speed, length, amplitude, phase)
/// @param instance_id
/// @param speed
/// @param length
/// @param amplitude
/// @param phase
function scr_earthbound_bg_set_wave_y() {
	...
	It will distort the image applying a sine wave to the y coordinates.
	...
	speed				real	The speed the sine wave moves left or right
	...
	------------------------------
		...
		scr_earthbound_bg_set_wave_y(layer_1, 1, 5, 0.01, 0);
	
	This will:
	- create an earthbound-background-instance,
	- sets its sprite to "spr_diamonds",
	- sets a relaxing, slow, small y-wave distortion

	-----------------------------------------------------------------------------*/

	with (argument[0]) {
		sin_wave_y_spd		= argument[1];
		sin_wave_y_len		= argument[2];
		sin_wave_y_amp		= argument[3];
		sin_wave_y_phase	= argument[4];
	}
